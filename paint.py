...
FONT_SIZES = [7, 8, 9, 10, 11, 12, 13, 14, 18, 24, 36, 48, 64, 72, 96, 144, 288]  # Размеры шрифта

MODES = [                                                                         # Режимы рисования, инструменты
...
CANVAS_DIMENSIONS = 1200, 700                                                     # Размеры поля

STAMP_DIR = './stamps'                                                            # Метод stamps.
STAMPS = [os.path.join(STAMP_DIR, f) for f in os.listdir(STAMP_DIR)]              # Открывает папку Яндекс.Штампов

SELECTION_PEN = QPen(QColor(0xff, 0xff, 0xff), 1, Qt.DashLine)                    # Выбор инструмента рисования
PREVIEW_PEN = QPen(QColor(0xff, 0xff, 0xff), 1, Qt.SolidLine)                     # Просмотр панели инструментов

# Storage = []

def build_font(config):                                                           # Настройка шрифта
    """
    Создайте полный шрифт из параметров конфигурации
    :param self:
    :param config:
    :return: QFont
    """
    font = config['font']                                                         # Параметр конфигурации шрифта
    font.setPointSize(config['fontsize'])                                         # Параметр размера шрифта
    font.setBold(config['bold'])                                                  # Параметр "жирного шрифта"
    font.setItalic(config['italic'])                                              # Параметр "курсивного шрифта"
    font.setUnderline(config['underline'])                                        # Параметр "подчеркивания шрифта"
    return font                                                                   


class Canvas(QLabel):                                                             # Отрисовка Холста

#   Storage = []

    mode = 'rectangle'                                                            # Холст имеет прямоугольную форму

    primary_color = QColor(Qt.black)                                              # Начальный цвет (в зависимости от темы. Тема - цвет холста, который может быть белым или черным)
    secondary_color = None

    primary_color_updated = pyqtSignal(str)
    secondary_color_updated = pyqtSignal(str)

    config = {                                                                    # Хранилище настроек конфигурации (ширина пера, текст и т. д.)
        # Опции рисования
        'size': 1,
        'fill': True,
        # Опции шрифта
    ...
    def initialize(self):
        self.Storage = []   # Хранилище скрытых холстов
    ...
    def Undo(self):
        ...
        p = QPainter(self.Storage[len(self.Storage) - 1])
    ...
    def stamp_mousePressEvent(self, e):
        self.Storage.append(self.pixmap())
        p = QPainter(self.pixmap())
        stamp = self.current_stamp
      # p.append(Storrage)
        p.drawPixmap(e.x() - stamp.width() // 2, e.y() - stamp.height() // 2, stamp)
